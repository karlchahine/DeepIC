...
from deepcomm.utils import dec2bitarray, bitarray2dec
...
def conv_encode(message_bits, trellis, puncture_matrix = None):
    ...
    for i in range(number_inbits): # Loop through all input bits
        ...
        # outputs = output_table[current_state][current_input]
        #current_outputs = torch.stack(list(map(lambda x, y: torch.from_numpy(dec2bitarray(output_table[x][y].item(), n)), current_states.long(), current_input.long())))
        ...
        # current_states = torch.stack(list(map(lambda x, y: next_state_table[x][y], current_states.long(), current_input.long())))
        current_states = next_state_table[current_states.long(), current_input.long()]
        j += 1
        # print(current_input[iii], current_states[iii], current_outputs[iii])
